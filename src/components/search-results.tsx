...
export const SearchResults: React.FC<Props> = ({ profiles, pageSize = 15 }) => {
  ...
  const [ page, pages, incrementPage, decrementPage, setPage, setPageCount ] = usePage(pageCount, 0);
      ...
      {pageCount > 1
        ? (
          <>
            ...
            <div className="mb-4">
              <button className="btn btn-primary btn-sm" onClick={() => decrementPage()}><IoIosArrowBack /></button>
              {pages.map(i => (
                // tslint:disable:jsx-key
                <button className={'btn btn-sm ' + (i === page ? 'btn-light' : 'btn-primary')} onClick={() => setPage(i)}>{i + 1}</button>
              ))}
              <button className="btn btn-primary btn-sm" onClick={() => incrementPage()}><IoIosArrowForward /></button>
            </div>
          </>
        )
        : null
